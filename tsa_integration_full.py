"""
TSA Integration — Full 4-motor-per-leg configuration.

Extends tsa_integration.py from one TSAActuator per leg to four independent
TSAActuator units per leg, mirroring the real hardware layout.

Motor layout
------------
Each leg has four TSA motors.  Two mount centrally over the knee axis
(lateral_offset_deg = 0) and two are slightly displaced in the frontal
plane — one dextral (positive α), one sinistral (negative α).  All four
cables terminate at adjacent fixed points on the proximal calf so all
contribute to knee extension torque.

Geometry correction
-------------------
A motor with frontal-plane offset angle α contributes less to sagittal-plane
knee extension than a centred motor:

    tau_knee_i = T_i × d × cos(α_i)                          (1)

where T_i is cable tension and d is the nominal sagittal moment arm.
For |α| < 15°, cos(α) > 0.966 — the correction is < 3.5% per motor.
The correction applies to the torque output only; the TSAActuator internally
uses d_nominal throughout (so its stall / EOM logic is unchanged).
Frontal-plane (ad/abduction) moments from symmetric ±α pairs cancel and are
not injected into the knee DOF.

Cable-slack model
-----------------
Cable path follows L(θ) = L₀ + r·θ (anterior knee geometry, eq. 2):

    X_geom = X₀ + r·max(0, θ_initial − θ_current)              (2)

where X₀ is the pretension contraction and r = MOMENT_ARM (anatomical
anterior-knee radius, constant for all motors regardless of lateral offset α).
The d_eff correction does NOT apply here — cable path geometry depends on
knee anatomy, not motor placement.

Staggered activation
--------------------
Each motor has an ``activation_time`` [s].  On the first step where
t >= activation_time the motor becomes active and stays active for the
remainder of the trial.  Activation is one-way during a trial; full_reset()
restores the initial state for a new trial.

Two control modes
-----------------
'full_power'  (recommended)
    Each active motor is commanded at its maximum torque (τ_stall at current
    speed).  Implemented by passing T_des = 1e6 N, which the actuator's
    internal clamp reduces to tau_cmd = tau_avail = τ_stall(1 − θ̇/ω₀).
    Total delivered torque grows as more motors activate and as J increases
    with winding.  Robust to the EOM under-delivery issue (HANDOFF_v3 Issue 1)
    because each motor is already doing the most it physically can.

'demand_share'
    The demanded knee torque is distributed equally among active motors.
    Each motor receives T_des_i = (τ_demand / N_active) / d_eff_i.
    Theoretically tracks demand but under-delivers during early wind-up
    because the EOM branch is inertia-dominated (see HANDOFF_v3 Issue 1).
    Included for comparison; full_power is preferred.

Resistance force sharing
------------------------
The joint resistance torque is a property of the joint, not of any one cable.
With N_active cables sharing the load at the calf attachment, each cable bears
1/N_active of the total resistance:

    F_resist_per_motor = F_resist_total / N_active

This lowers each motor's stall threshold so motors spend more time in the EOM
(advancing) branch as N grows — physically correct, since N motors together
are less likely to individually stall against the shared load.

Drop-in compatibility
---------------------
TSAIntegrationFull has the same step(t, dt, tau_r, tau_l) and reset()
signature as TSAIntegration.  To use it, replace:
...
No changes needed to the step() call site in sts_ctrl.py.
"""
