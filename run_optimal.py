...
# ── Policy path ───────────────────────────────────────────────────────────────
POLICY_PATH = "logs/ppo_v7/ppo_tsa_final"
# POLICY_PATH = None
...
def _print_summary(t_base, ph_base, acts_base, base_completed,
    ...
    print("\n" + "=" * 68)
    print("QUADRICEPS ACTIVATION COMPARISON")
    print("=" * 68)
    ...
    print("-" * 68)
    print(f"{'':14s}  {'Baseline':>10s}  {'TSA Exo':>10s}  {'Reduction':>10s}  {'Duration base':>13s}  {'Duration exo':>12s}")
    print("-" * 68)

    m_base = acts_base_w.mean()
    m_exo  = acts_exo_w.mean()
    pct    = 100 * (m_base - m_exo) / (m_base + 1e-9)
    ...
    for p in sorted(set(ph_base) | set(ph_exo)):
        ...
        pct_p = 100 * (mb - me) / (mb + 1e-9) if np.isfinite(mb) and np.isfinite(me) else float("nan")
...
def _plot_comparison(t_base, ph_base, acts_r_base, acts_l_base,
    ...
    fig.suptitle("Quadriceps Activation: Baseline vs TSA Exoskeleton",
    ...
    for ax, title, r_base, l_base, r_exo, l_exo in datasets:
        ...
        overall_red = 100 * (mb_w - me_w) / (mb_w + 1e-9) if np.isfinite(mb_w) and np.isfinite(me_w) else float("nan")
