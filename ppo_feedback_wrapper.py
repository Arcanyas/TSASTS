"""
Feedback PPO wrapper for sit-to-stand assistance.

This is different from the previous one-step TSA-parameter PPO wrapper.
Here PPO is a real feedback controller:

    observation_t = skeleton/body state at current simulation step
    action_t      = right/left knee-assist command in [0, 1]
    reward_t      = per-step progress/tracking/muscle/assist/stability reward

The biological/reflex STS controller is kept, but lower-limb output can be
scaled to mimic reduced leg-control capacity, e.g. leg_scale=0.80.
PPO assistance is an idealised knee-extension torque, not the full TSA geometry.
Use this first to test whether feedback RL can improve the reduced-control STS.

Expected location:
    project_root/ppo_feedback_wrapper.py

Run via:
    mjpython train_ppo_feedback.py
"""
...
def apply_leg_control_reduction(params: STSReflexParams, scale: float = 0.90) -> STSReflexParams:
    """
    Controller-level reduced lower-limb capacity.

    This scales the reflex-to-stimulation mapping for leg groups only. It does
    not change the MuJoCo muscle force parameters. That makes the experiment a
    reduced leg-control condition rather than a physiological weakness model.
    """
...
class STSFeedbackAssistEnv(gymnasium.Env):
    ...
    PPO observes the skeleton at each timestep and outputs idealised assistive
    knee-extension commands for the right and left knees.

    Action:
        [assist_r, assist_l] in [0, 1]

    Observation:
        normalised vector of phase, body heights/velocities, lean/pitch, lower
        limb joint angles/velocities, contacts and loads.
    """
    ...
    def _controller_step_with_assist(self, action: np.ndarray, phase: int) -> Tuple[float, float]:
        """
        Reproduces the important parts of SitToStandSim.step(), but inserts
        PPO knee torque before the underlying env.step(action).

        Do not call self.sts.step() here: it clears forces and advances the sim
        before external PPO torque can be added.
        """
        ...
        # Controller-applied generalized/body forces.
        if hasattr(sts.sim.data, "qfrc_applied"):
            if phase == 1:
        ...
        # PPO idealised knee-extension assist torque. This must happen after the
        # controller clears qfrc_applied and before env.step().
    ...
    def _safe_env_step(self, action: np.ndarray) -> None:
        """Handle old Gym 4-tuples and Gymnasium 5-tuples without caring."""
    ...
    def _compute_reward(
        ...
        # Progress: reward upward pelvis progress, especially before standing.
        dy = pelvis_y - self._prev_pelvis_y
        r_progress = 15.0 * dy + 0.02 * max(phase - self._prev_phase, 0)
        self._prev_pelvis_y = pelvis_y
        self._prev_phase = phase

        # Posture/tracking toward sane STS targets, not exact baseline imitation.
        if phase <= 1:
            pelvis_target = 0.55
            torso_target = 0.62
            lean_target = 0.50
        ...
        # Penalise violent movement and collapse.
        ...
        # Muscle effort: mainly VAS/leg activation. PPO should help 70% legs do
        # the task without simply increasing biological stimulation.
        ...
        # Exoskeleton effort. Small penalty prevents max-torque all the time.
        ...
        # Left/right action symmetry mild regulariser.
        ...
        if phase >= 4:
            # Small timing target so PPO does not learn instant launches or very slow stands.
    ...
    def _is_bad_failure(self, obs: Dict[str, float], phase: int) -> bool:
        ...
        if self._step_count < 20:
            return False
        if phase >= 2 and not grounded:
            # A brief contact glitch is okay; persistent failure is handled by reward.
            return False
