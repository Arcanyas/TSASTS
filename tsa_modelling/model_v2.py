...
class TSASimulator:
    """
    Single-unit Twisted String Actuator (TSA) simulator.

    Forward-dynamics formulation: given motor torque input τ(t), integrate
    motor angle θ forward in time, then derive all task-space quantities via
    the kinematic and dynamic models in Section 1.4.

    Motor equation of motion (joint space, eq. 1.22 / 1.32 top row):
        I·θ̈ = τ_cmd  -  J·T  -  b_θ·θ̇

    where string tension T is solved from the payload equation (eq. 1.32 bottom):
        T = m·Ẍ + F_ext + F_fr_payload
          = m·(J·θ̈ + J̇·θ̇)  +  F_ext  +  b_X·Ẋ

    Substituting T back eliminates the algebraic loop and gives a single ODE
    for θ̈ that is stable under Euler integration provided dt is small enough.

    String tension is physically non-negative (strings can only pull).
    """
    ...
    def _eom(
        ...
        """
        Solve for θ̈, T, and Ẍ simultaneously.

        From the coupled motor+payload equations (eq. 1.32):
            I·θ̈  =  τ_cmd  -  J·T  -  b_θ·θ̇          ... (motor)
            T     =  m·Ẍ + F_ext + b_X·Ẋ               ... (payload)
            Ẍ     =  J·θ̈ + J̇·θ̇                         ... (acceleration kinematics)

        Substituting Ẍ and T into the motor equation gives:
            I·θ̈  =  τ_cmd  -  J·(m·(J·θ̈ + J̇·θ̇) + F_ext + b_X·J·θ̇)  -  b_θ·θ̇
            (I + m·J²)·θ̈  =  τ_cmd  -  J·(m·J̇·θ̇ + F_ext + b_X·J·θ̇)  -  b_θ·θ̇

        This is eq. 1.22 (inertia D_θ·θ̈ = τ - ...) — fully explicit, no loop.

        F_ext_override, if provided, bypasses the gravity_along_string flag so
        the caller can pass the actual joint resistance force while keeping self.m
        as the true cable-side inertia.
        """
    ...
    def simulate(
        ...
        """
        Integrate motor dynamics forward in time using RK4.
        ...
        dt : float
            Time step [s].  Recommend ≤ 1 ms for stability.
        verbose : bool
            Print progress every 10 % of simulation.

        Returns
        -------
        dict with keys: t, theta, theta_dot, X, X_dot, X_ddot,
                        J, J_dot, tau_cmd, tau_available,
                        tau_required, T, stalled
        """
