"""
TSA hardware parameter optimisation — Gymnasium environment wrapper.

Design: one-step episodic environment.
    reset()      → returns dummy observation (zeros)
    step(action) → runs a full STS episode with fixed θ, returns scalar reward

Action space (symmetric, 5-D):
    θ = [L, t0, t1, t2, t3]  — L in [L_min, L_max], t_i in [t_min, t_max]
    Ordering constraint t0 ≤ t1 ≤ t2 ≤ t3 enforced by sorting, not clipping.

Reward (three components, see ppo_config.RewardParams):
...
class TSAOptimEnv(gymnasium.Env):
    """
    One-step episodic Gymnasium env for TSA hardware parameter optimisation.

    Each call to step(action) runs a complete STS simulation episode with
    fixed parameters θ and returns a scalar reward.  terminated=True is
    always returned so SB3 starts a new episode immediately.

    Parameters
    ----------
    mujoco_env : gym environment returned by gym.make('TorsoLegs')
    ...
    def _resolve_muscle_ids(self, sts: SitToStandSim) -> None:
        ...
        # Cache knee DOF addresses for τ_demand computation.
    ...
    def _run_episode(self, sts: SitToStandSim) -> float:
        """Run one STS episode and return the scalar reward."""
        dt      = float(sts.sim.model.opt.timestep)
        T_ep    = self.cfg.max_steps * dt

        torque_deficit_acc = 0.0  # ∫ max(α·τ_dem − τ_del, 0) dt  — one-sided: penalise under-delivery only
        muscle_acc         = 0.0  # ∫ mean_quad_activation dt
        t_final            = T_ep  # updated on Phase 4 entry (STS complete); defaults to full length
        sts_completed      = False
        ...
        for _step in range(self.cfg.max_steps):
            ...
            # Phase 4 = STS complete: record completion time and terminate.
            if phase >= 4:
                t_final       = float(sts.sim.data.time)
                sts_completed = True
                break

            # ── Torque tracking (one-sided deficit) ──────────────────
            # Only penalise when delivered torque < target; over-delivery is free.
            # This prevents reward hacking by late/no activation (tau_del=0 always
            # incurs the full target as deficit, so the PPO must actually fire).
        ...
        # Normalise using actual elapsed time (t_final); penalise longer episodes.
