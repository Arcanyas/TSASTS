"""
Minimal-control PPO wrapper for TSA sit-to-stand optimisation.

This keeps the same high-level design as the original PPO wrapper:
    action = [L, t0, t1, t2, t3]
    one PPO step = one full sit-to-stand rollout

But the reward is changed from target torque support to:
    1) track the working baseline sit-to-stand kinematics,
    2) reduce biological knee-extensor / VAS effort,
    3) avoid excessive TSA torque,
    4) require stable completion.

Use by changing train_ppo.py to import this environment, or use the companion
train_ppo_min_control.py script.
"""
...
class TSAOptimMinControlEnv(gymnasium.Env):
    """
    One-step Gymnasium env for minimal-control TSA parameter optimisation.

    PPO still chooses the TSA hardware/schedule parameters:
        theta = [L, t0, t1, t2, t3]

    A reference sit-to-stand trajectory is generated once using the working
    baseline controller with no TSA. Each PPO rollout is then rewarded for
    staying close to that reference while reducing VAS / quadriceps activation.
    """
    ...
    def _resolve_target_muscle_ids(self, sts: SitToStandSim) -> List[int]:
        """
        Target the muscles most directly relieved by knee-extension TSA assistance.

        VAS is the primary target. RF is deliberately not assumed because this
        controller groups VAS cleanly, whereas RF is biarticular and model naming
        can vary.
        """
        ids: List[int] = []
    ...
    def _run_episode(self, sts: SitToStandSim) -> Dict[str, float]:
        ...
        for _step in range(int(self.cfg.max_steps)):
            ...
            # Penalise excessive TSA torque gently. This prevents the trivial
            # solution where PPO removes biological effort by using absurd exo effort.
            if sts.tsa is not None:
            ...
            # Soft online safety/stability penalties.
        ...
        # Final posture / success term. Phase 5 means your controller judged the
        # movement stable enough after Phase 4.
        R_success = 1.0 if success else -1.0

        # Penalise duration deviation from the baseline/reference rather than
        # making "faster is always better".
        ref_end = self._reference_end_time(default=T_ep)
        R_time = -abs(t_final - ref_end) / max(ref_end, dt_default)

        # Conservative default weights. Adjust these in-code first; once stable,
        # move them into ppo_config.py if desired.
    ...
    def _reference_end_time(self, default: float) -> float:
        ...
        # Sum phase durations approximately using the last elapsed time in each phase.
        total = 0.0
