"""
TSA integration module for SitToStandSim.

Bridges tsa_modelling/actuator.py (single-unit TSA physics) with the MuJoCo
STS environment.  Owns actuator construction, per-step updates, and
qfrc_applied injection at each knee DOF.

Convention
----------
The TSA assists knee *extension* (reduces knee_angle toward 0).  MuJoCo's
hinge convention is that positive generalised force increases the joint angle
(i.e. drives flexion), so the assistive torque is injected as a *negative*
qfrc_applied at the knee DOF.

Effective inertia
-----------------
payload_mass is updated every step from the (knee, knee) diagonal of MuJoCo's
joint-space inertia matrix M.  This captures the full body's generalised
inertia at the knee DOF, not just the shin segment.  The diagonal element is
read cheaply as sim.data.qM[dof_Madr[dadr]] — no full matrix inversion needed.

Resistance force
----------------
F_resist = (qfrc_bias[dadr] + qfrc_constraint[dadr]) / d
qfrc_bias   = gravity + Coriolis at knee DOF
qfrc_constraint = constraint forces (includes body-weight load transmitted
                  through the foot contact).  Together they give the total
                  passive torque the cable must overcome.
"""
...
def build_tsa_actuator(side: str, L: float) -> TSAActuator:
    """
    Construct one TSAActuator for the given side.

    Parameters
    ----------
    side : 'right' or 'left'
    L    : untwisted string length [m] — placeholder, will be optimised.
    """
...
class TSAIntegration:
    ...
    Call ``step()`` inside SitToStandSim.step() after clearing qfrc_applied
    and before env.step(action).

    Example
    -------
    ::

        tau_r, tau_l = self._tsa_knee_torque_demand(obs, phase)
        self.tsa_integration.step(t, dt, tau_r, tau_l)
    """
